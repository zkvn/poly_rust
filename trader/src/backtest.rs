...
pub fn load_binance(path: &str) -> Result<Vec<BinanceRow>> {
    ...
    for batch in reader {
        ...
        let price_col = batch.column_by_name("binance")
            .with_context(|| "missing column binance")?
            .as_any().downcast_ref::<Float64Array>()
            .with_context(|| "binance not Float64")?;
        let slug_col = batch.column_by_name("slug")
...
pub fn load_poly(path: &str) -> Result<Vec<PolyRow>> {
    ...
    for batch in reader {
        ...
        let ts_col = batch.column_by_name("ts")
            ...
            .as_any().downcast_ref::<Float64Array>()
            .with_context(|| "ts not Float64")?;
        let up_col = batch.column_by_name("up")
            .with_context(|| "missing column up")?
            .as_any().downcast_ref::<Float64Array>()
            .with_context(|| "up not Float64")?;
        let dn_col = batch.column_by_name("dn")
        ...
        let slug_col = slug_col.as_ref();
        for i in 0..batch.num_rows() {
            if ts_col.is_null(i) || up_col.is_null(i) || dn_col.is_null(i) || is_null_str(slug_col, i) {
                continue;
            }
            rows.push(PolyRow {
                ts: ts_col.value(i),
...
pub fn filter_by_date(
    ...
    let valid_slugs: std::collections::HashSet<String> = p_rows.iter()
        .filter_map(|r| {
            let slug_ts: f64 = r.slug.rsplit('-').next()?.parse().ok()?;
            if slug_ts >= ts_start && slug_ts <= ts_end { Some(r.slug.clone()) } else { None }
        })
        .collect();

    let b = b_rows.into_iter().filter(|r| valid_slugs.contains(&r.slug)).collect();
    let p = p_rows.into_iter().filter(|r| valid_slugs.contains(&r.slug)).collect();
    Ok((b, p))
}
...
fn merge_ticks(b_cycle: &[BinanceRow], p_cycle: &[PolyRow]) -> Vec<MergedTick> {
    ...
    keyed.into_iter().map(|(_ts, _ins, is_poly, idx)| {
        if is_poly {
...
pub fn run_backtest(
    ...
    let mut machines: Vec<Machine> = params.strategies.iter().map(|name| {
        match name.as_str() {
...
pub fn load_price_data(asset: &str, date: &str, prices_dir: &str) -> Result<(Vec<BinanceRow>, Vec<PolyRow>)> {
...
mod tests {
    ...
    fn btc_20260620_golden() {
        ...
        let (b_rows, p_rows) = load_price_data("BTC", "2026-06-20", prices_dir)
            .expect("load price data");
    ...
    fn hkt_session_boundary() {
        ...
        let ts = hkt_offset.with_ymd_and_hms(2026, 6, 20, 1, 0, 0).unwrap().timestamp() as f64;
    ...
    fn merge_ticks_binance_first_at_equal_ts() {
        let b = vec![BinanceRow { ts: 1000.0, price: 50000.0, slug: "s".into() }];
        let p = vec![PolyRow { ts: 1000.0, up: 0.6, dn: 0.4, slug: "s".into() }];
        let merged = merge_ticks(&b, &p);
        assert_eq!(merged.len(), 2);
        assert!(matches!(merged[0], MergedTick::Binance(_)), "binance must be first at equal ts");
