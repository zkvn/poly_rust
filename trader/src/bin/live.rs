...
struct Args {
    ...
    /// Directory for per-asset trade logs/state files (live_trades_<asset>.csv, live_state_<asset>.json).
...
/// Everything one asset's cycle needs, mutated in place as ticks/events arrive.
/// `worker` lives inside so `process_actions`/`execute` only need one `&mut`
/// borrow (splitting `worker` and its sibling fields into two separate
/// borrows from the same `HashMap` entry is otherwise awkward).
struct AssetSlot {
    worker: Worker,
    params: AssetParams,
    up_id: U256,
...
impl Driver<'_> {
    ...
    /// / PNL sections with one row per asset, sorted for determinism.
    async fn render_status(&self, assets: &HashMap<String, AssetSlot>) -> String {
            ...
            for action in &actions {
                match action {
                    ...
                    Action::LogTrade(rec) => {
                        ...
                        self.notify(&format!("💰 <b>{}</b> {summary}", slot.worker.asset)).await;
                    }
                    _ => {
...
async fn main() -> Result<()> {
    ...
    println!(
        "[live] assets={} strategy={} size_usdc=${:.2} max_trades={} log_dir={}",
        args.asset.join(","), args.strategy, args.size_usdc, args.max_trades, args.log_dir
    );
    ...
    let driver = Driver { engine: &engine, telegram: telegram_send.clone() };
    driver
        .notify(&format!(
            "🟢 live driver started: <b>{}</b> (strategy={}, size=${:.2}, max_trades={})",
            args.asset.join(", "), args.strategy, args.size_usdc, args.max_trades
        ))
        .await;
    ...
    loop {
        if assets.values().all(|s| s.trades_completed >= args.max_trades) {
        ...
        tokio::select! {
            Some((asset, tick)) = binance_rx.recv() => {
                if let Some(slot) = assets.get_mut(&asset) {
            ...
            _ = heartbeat.tick() => {
                ...
                for (asset, slot) in &assets {
                    if let Some(slug) = &slot.current_slug {
                        println!("[live] heartbeat {asset} slug={slug} T-{time_left:.0}s binance={:.4} up={:.4} dn={:.4}",
                            slot.last_binance, slot.last_poly_up, slot.last_poly_dn);
                    }
                }
            }
            ...
            _ = ticker.tick() => {
                ...
                if slot_now != current_slot_val {
                    ...
                    for asset in &args.asset {
                        let slot = assets.get_mut(asset).expect("every --asset has a slot");
                        if slot.current_slug.is_some() {
                        ...
                        match fetch_meta(&http, &slug).await {
                            Ok((u, d)) => {
                                ...
                                println!("[live] new cycle {asset} slug={slug} open_binance={}", slot.last_binance);
            ...
            Some(text) = telegram_rx.recv() => {
                ...
                let reply = match cmd {
                    ...
                    Command::Halt { asset } => {
                        match assets.iter_mut().find(|(a, _)| a.eq_ignore_ascii_case(&asset)) {
                            Some((_, slot)) => {
                                slot.worker.step(Event::Control(ControlEvent::Halt));
                                Some(format!("🛑 Halted {asset} — new entries suppressed, open positions still managed."))
                            }
                            None => Some(format!("this driver doesn't manage {asset} — trading {}", args.asset.join(", "))),
                        }
                    }
