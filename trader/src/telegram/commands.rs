...
fn dict_params() -> HashSet<&'static str> {
    [
        "halt_prob", "halt_rev", "halt_reset_hour_rev", "halt_reset_hour_hp",
        "sl_high_prob", "sl_reversal", "delta_pct_hp", "delta_pct_rev",
        "price_low", "price_high", "price_high_rev",
        "reversal", "reversal_low_threshold", "trade_size_usdc", "enter_when_time_left",
    ]
    .into_iter()
    .collect()
}
...
pub enum Command {
    ...
    StrategiesQuery,
    StrategiesSet { asset: String, strategies: Vec<String> },
    Markets,
    DeltaQuery,
    DeltaSet { strat: DeltaStrat, key: String, value: String },
    ReconToday { asset: Option<String>, date: Option<String> },
    WarningAnalysis { date: Option<String> },
    Sys,
    Last(usize),
    LiveLog(usize),
    SkippedLog(usize),
    Status,
    Params,
    Set { param: String, key: String, value: String },
    ...
    Some(match cmd.as_str() {
        ...
        "/strategies" => {
            if parts.len() == 1 {
                ...
                let invalid: Vec<&String> = names.iter().filter(|n| !valid.contains(n.as_str())).collect();
                if !invalid.is_empty() {
                    ...
                    Command::StrategiesSet { asset, strategies: names }
                }
            }
        }
        ...
        "/recon_today" => {
            let asset = parts[1..].iter().find(|p| p.parse::<u64>().is_err()).map(|s| s.to_uppercase());
            let date = parts[1..].iter().find(|p| p.parse::<u64>().is_ok()).map(|s| s.to_string());
            Command::ReconToday { asset, date }
        }
        "/warning_analysis" => {
            Command::WarningAnalysis { date: parts.get(1).map(|s| s.to_string()) }
        }
        "/sys" => Command::Sys,
        ...
        c if c.starts_with("/live_log") => {
            let n = parts.get(1).and_then(|s| s.parse::<usize>().ok()).unwrap_or(40);
            Command::LiveLog(n)
        }
        ...
        "/set" => {
            if parts.len() < 3 {
                Command::Invalid(format!(
                    ...
                    { let mut v: Vec<_> = settable_params().into_iter().collect(); v.sort(); v.join(", ") }
                ))
            } else {
                ...
                if !settable_params().contains(param.as_str()) {
                    ...
                    Command::Set { param, key: parts[2].to_uppercase(), value: parts[3].to_string() }
                } else {
                    Command::Set { param, key: "default".to_string(), value: parts[2].to_string() }
                }
            }
        }
        "/reset_losses" => {
            Command::ResetLosses { asset: parts.get(1).map(|s| s.to_uppercase()).unwrap_or_default() }
        }
        "/halt" => Command::Halt { asset: parts.get(1).map(|s| s.to_uppercase()).unwrap_or_default() },
        "/resume" => Command::Resume { asset: parts.get(1).map(|s| s.to_uppercase()).unwrap_or_default() },
...
mod tests {
    ...
    fn parses_set_default_scalar() {
        assert_eq!(
            ...
            Some(Command::Set { param: "trade_size_usdc".to_string(), key: "default".to_string(), value: "2.0".to_string() })
        );
    }
    ...
    fn parses_set_per_asset_dict_param() {
        assert_eq!(
            ...
            Some(Command::Set { param: "halt_rev".to_string(), key: "BTC".to_string(), value: "3".to_string() })
        );
    }
    ...
    fn set_resolves_aliases() {
        assert_eq!(
            ...
            Some(Command::Set { param: "delta_pct_rev".to_string(), key: "BTC".to_string(), value: "0.0005".to_string() })
        );
    }
    ...
    fn parses_halt_resume_scoped_and_global() {
        assert_eq!(parse_command("/halt"), Some(Command::Halt { asset: "".to_string() }));
        assert_eq!(parse_command("/halt btc"), Some(Command::Halt { asset: "BTC".to_string() }));
        assert_eq!(parse_command("/resume ETH"), Some(Command::Resume { asset: "ETH".to_string() }));
    }

    #[test]
    fn parses_reset_losses() {
        assert_eq!(parse_command("/reset_losses"), Some(Command::ResetLosses { asset: "".to_string() }));
        assert_eq!(parse_command("/reset_losses doge"), Some(Command::ResetLosses { asset: "DOGE".to_string() }));
    }
    ...
    fn parses_delta_default_and_scoped() {
        assert_eq!(
            ...
            Some(Command::DeltaSet { strat: DeltaStrat::HighProb, key: "default".to_string(), value: "0.0004".to_string() })
        );
        assert_eq!(
            parse_command("/delta rev BTC 0.0005"),
            Some(Command::DeltaSet { strat: DeltaStrat::Reversal, key: "BTC".to_string(), value: "0.0005".to_string() })
        );
    }

    #[test]
    fn parses_trade_assets_query_and_set() {
        assert_eq!(parse_command("/trade_assets"), Some(Command::TradeAssetsQuery));
        assert_eq!(
            parse_command("/trade_assets BTC, ETH, doge"),
            Some(Command::TradeAssetsSet(vec!["BTC".to_string(), "ETH".to_string(), "DOGE".to_string()]))
        );
    }
