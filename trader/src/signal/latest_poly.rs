...
impl LatestPolySignal {
    pub fn new() -> Self {
        Self { up: 0.0, dn: 0.0, ts: 0.0 }
    }

    pub fn up(&self) -> f64 { self.up }
    pub fn dn(&self) -> f64 { self.dn }

    /// Age of the last poly tick relative to `now` (seconds).
    /// Returns +inf if never received.
    pub fn age(&self, now: f64) -> f64 {
        if self.ts > 0.0 { now - self.ts } else { f64::INFINITY }
    }
}

impl Signal for LatestPolySignal {
    fn name(&self) -> &str { "latest_poly" }
...
mod tests {
    ...
    fn latest_poly_tracks_price() {
        ...
        s.on_poly(PolyTick { ts: 100.0, up: 0.85, dn: 0.15 });
    ...
    fn spread_signal() {
        ...
        s.on_poly(PolyTick { ts: 100.0, up: 0.85, dn: 0.16 });
